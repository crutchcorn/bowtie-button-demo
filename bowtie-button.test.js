...
function createHarness({ pointerCaptureThrows = false } = {}) {
  ...
  function flushFrame(performanceTimestamp = clock, rafTimestamp = performanceTimestamp) {
...
describe('production bundle contract', () => {
  it('keeps all test seams out of the shipped JavaScript', () => {
    ...
    expect(gzipSync(normalizedScript, { level: 9 }).byteLength).toBeLessThanOrEqual(
      MAX_RUNTIME_GZIP_BYTES
    );
  });
});
...
describe('sprite contract', () => {
  ...
  it('packages the exact ten frame states into one compact SVG sprite', () => {
    ...
    expect(readdirSync('assets').filter(file => /^frame\d+\.svg$/.test(file))).toEqual([]);
